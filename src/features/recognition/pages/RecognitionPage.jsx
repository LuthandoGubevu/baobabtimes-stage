 ...
 * RecognitionPage
 *
 * Section order (per UX requirement):
 *  1. Page header + "Recognize a Peer" button
 *  2. Latest Recognitions  ← primary focus
 *  3. Value Summary        ← compact horizontal strip
 *  4. Monthly Spotlight    ← compact leaderboard panels
 */
    ...
    <div className="space-y-12 pb-20">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-stone-200 pb-8">
        <div className="max-w-2xl">
          <h1 className="text-5xl font-serif font-bold mb-4 italic">
            Recognition Wall
          </h1>
          <p className="text-stone-500 text-lg font-light">
            Celebrating the achievements, teamwork, and innovation of our
            incredible people.
          </p>
        </div>
        ...
        <button
          ...
          className="flex items-center space-x-2 px-6 py-3 bg-stone-900 text-white font-bold rounded-full hover:bg-stone-800 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 shrink-0"
        >
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-stone-400 shrink-0">
        ...
        {isLoadingRecognitions ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="bg-white p-8 rounded-[2.5rem] border border-stone-200 h-64 animate-pulse flex flex-col justify-between"
              >
          ...
          <div className="text-center py-20 bg-white rounded-[3rem] border border-stone-200">
            <p className="text-stone-500">
              Failed to load recognitions. Please try again later.
            </p>
          </div>
          ...
          <div className="text-center py-32 bg-white rounded-[3rem] border border-stone-200 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-stone-500 max-w-xs mx-auto">
                Be the first to celebrate a colleague's contribution to the team!
              </p>
            </div>
            <button
              onClick={() => executeProtectedAction(() => setIsModalOpen(true))}
      ...
      <div className="space-y-6">
        ...
        {/* Horizontally scrollable on mobile/tablet, grid on desktop */}
        <div className="relative">
          <div className="flex lg:grid lg:grid-cols-6 items-center gap-3 overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0 -mx-4 px-4 sm:mx-0 sm:px-0 scroll-smooth">
            {recognitionValues.map((value) => (
              <RecognitionSummaryCard
                ...
                className="flex-shrink-0 lg:min-w-0"
              />
            ))}
          </div>
          {/* Scroll fade — mobile only */}
      ...
      {/* ── 4. MONTHLY SPOTLIGHT (compact leaderboards) ────────── */}
      <SpotlightSection />

      {/* Recognition submission modal */}
      <RecognitionModal
        isOpen={isModalOpen}
